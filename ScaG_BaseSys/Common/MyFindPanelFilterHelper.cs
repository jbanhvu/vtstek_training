...
namespace CNY_BaseSys.Common
{
	public class MyFindPanelFilterHelper
	{
		...
		public bool IsPerFormEvent
		{
			set
			{
				if (value && !this._isPerFormEvent)
				{
					...
					BandedGridView bgView = _view as BandedGridView;
					if (bgView == null)
					{
					...
					{
						bgView.BandWidthChanged += bgView_BandWidthChanged;
						bgView.CustomDrawBandHeader += bgView_CustomDrawBandHeader;
		...
		public bool IsBestFitDoubleClick
		{
			set
			{
				if (value && !this._isBestFitDoubleClick)
				{
					...
					{
						bgView.DoubleClick += bgView_DoubleClick;
						bgView.Click += bgView_Click;
						bgView.DragObjectStart += bgView_DragObjectStart;
		...
		public MyFindPanelFilterHelper(GridView view, bool isAndSearchNew=false)
		{
			_view = view;
		...
		private void bgView_Click(object sender, EventArgs e)
        {
            ...
            List<BandedGridColumn> lCol = new List<BandedGridColumn>();
            Point pt = gc.PointToClient(Control.MousePosition);
            BandedGridHitInfo hi = gv.CalcHitInfo(pt);
            if (hi.InBandPanel)
            {
                GridBand gBand = hi.Band;
                if (gBand == null) return;
                if (gBand.HasChildren)
                {
                    lCol = gBand.GetAllVisibleColumnInGridBandMinLevel().ToList();
                }
                else
                {
                    lCol.Add(gBand.Columns[0]);
                }
            ...
            if (lCol.Count <= 0) return;



            var q1 = gv.GetSelectedCells().Select(p => (BandedGridColumn)p.Column).Distinct().ToList();

            var qJoin = q1.Join(lCol, p => p.FieldName, t => t.FieldName, (p, t) => p.VisibleIndex).ToList();
            var q2 = qJoin.Any();
            gv.ClearSelection();
            BandedGridColumn endCol = lCol.Last();
            BandedGridColumn beginCol = lCol.First();

            gv.FocusedRowHandle = 0;
            gv.FocusedColumn = endCol;
            ...
            if (q2)
            {
                if (q1.Any())
                {
                    var q3 = q1.Union(lCol).OrderBy(p => p.VisibleIndex).ToList();
                    gv.SelectCells(0, q3.First(), lastRow, q3.Last());
                }
                else
                {
                    gv.SelectCells(0, beginCol, lastRow, endCol);
                }
            }
            else
            {
		...
		private void bgView_DoubleClick(object sender, EventArgs e)
        {
            var gv = sender as BandedGridView;
            if (gv == null) return;
		...
		private void bgView_DragObjectStart(object sender, DragObjectStartEventArgs e)
		{
			var gv = sender as BandedGridView;
			if (gv == null) return;
			if (_allowSort)
		...
		private void bgView_BandWidthChanged(object sender, BandEventArgs e)
		{
			...
			int width = e.Band.Width;
			var q1 = gv.GetSelectedCells().Select(p => (BandedGridColumn)p.Column).Distinct().ToList();

		

			var q2 = e.Band.Columns.Cast<BandedGridColumn>().ToList();

			var q3 = q1.Join(q2, p => p, t => t, (p, t) => new
			{
				p,
				t
			}).ToList();
			...
			var q4 = q1.Where(p => q2.All(t => t != p)).ToList();
			foreach (BandedGridColumn col in q4)
			{
				col.Width = width;
			}
		}
		...
		private void bgView_CustomDrawBandHeader(object sender, BandHeaderCustomDrawEventArgs e)
		{
			if (!_isPerFormEvent) return;
			if (e.Band == null) return;
			var gv = sender as BandedGridView;
			...
			List<GridBand> lBand = gv.GetAllBandGridViewSelectedColumn();
			if (lBand.Any(p => p == e.Band))
			{
				...
				{
					e.Appearance.DrawString(e.Cache, caption.Replace("<b>", "").Replace("</b>", "").Trim().Replace("<br>", "\n"), e.Info.CaptionRect);
				}
		...
		private void view_CustomRowFilter(object sender, RowFilterEventArgs e)
		{
			...
			e.Handled = true;
			e.Visible = evaluator.Fit(((DataView)_view.DataSource)[e.ListSourceRow]);
		}
		#endregion
		...
		private void OnFixedClick(object sender, EventArgs e)
		{
			...
			var gv = _view as BandedGridView;
			...
			if (gv != null)
			{
				...
				if (!info.Checked)
				{
					...
					{
						BandedGridColumn bgColF = gv.VisibleColumns[i] as BandedGridColumn;
						if (bgColF == null) continue;
						GridBand gBandF = GetParentBand(bgColF.OwnerBand);
				   
						if (bgColF.FieldName == info.Column.FieldName)
						{
							gBandF.Fixed = FixedStyle.Left;
							break;
						}
						gBandF.Fixed = FixedStyle.Left;
					}
				...
				else if (info.Checked)
				{
					...
					for (int i = gv.VisibleColumns.Count - 1; i >= 0; i--)
					{
						BandedGridColumn bgCol = gv.VisibleColumns[i] as BandedGridColumn;
						if (bgCol == null) continue;
						GridBand gBand = GetParentBand(bgCol.OwnerBand);
						gBand.Fixed = FixedStyle.None;
					}
		...
		private GridBand GetParentBand(GridBand gBand)
		{
			GridBand parentBand = gBand.ParentBand;
			if (parentBand == null)
				return gBand;
			return GetParentBand(parentBand);
