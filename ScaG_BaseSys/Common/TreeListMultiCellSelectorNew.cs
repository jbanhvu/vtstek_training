...
namespace CNY_BaseSys.Common
{
    ...
    public class TreeListMultiCellSelectorNew
    {
        //------

        #region "Property"
        public event GetFixedColumnFieldNameHandler OnGetFixedColumn = null;
        private readonly FindVnTreeList _treeList;


        public FindVnTreeList TreeListMain
        {
        ...
        private List<string> _lColVn = new List<string>();
        public List<string> LColVn
        {
            get { return this._lColVn; }
            set { this._lColVn = value; }
        }
        ...
        #region "Contructor"
        public TreeListMultiCellSelectorNew(FindVnTreeList treeList, bool isAndSearchNew = false)
        {
          ...
          //  _treeList.OptionsBehavior.EditingMode = TreeListEditingMode.Inplace;
            ...
            _treeList.OnFindPanelVisibleChanged += treeList_OnFindPanelVisibleChanged;
            //VneseFindPanel findPanelnew = (VneseFindPanel)_treeList.FindPanelNew;
            //findPanelnew.OnFindPanelVisibleChanged += FindPanelNew_OnFindPanelVisibleChanged;


            _treeList.BeginFiltering += treeList_BeginFiltering;
            _treeList.EndFiltering += treeList_EndFiltering;
        ...
        private ExpressionEvaluator GetExpressionEvaluator(CriteriaOperator criteria, bool isReload, DataView dv)
        {
            if (!isReload)
            {
        ...
        private void treeList_OnFindPanelVisibleChanged(object sender, EventArgs e)
        {
        ...
        private void GetDataSearch(TreeList tl)
        {
            DataTable dtFt = tl.DataSource as DataTable;
        ...
        private void treeList_BeginFiltering(object sender, EventArgs e)
        {
        ...
        private bool _isGetDataSearch = true;

        private void treeList_EndFiltering(object sender, EventArgs e)
        {
            TreeList tl = sender as TreeList;
            if (tl == null) return;
            if (!tl.FindPanelVisible) return;
            string findText = tl.FindFilterText;
            if (string.IsNullOrEmpty(findText)) return;
            _isGetDataSearch = true;
        }
        ...
        private void treeList_FilterNode(object sender, FilterNodeEventArgs e)
        {
            ...
            string findText = tl.FindFilterText.ToLower();
            if (string.IsNullOrEmpty(findText)) goto finished;
            e.Handled = true;
            if (_isGetDataSearch)
            {
                GetDataSearch(tl);
                _isGetDataSearch = false;
               
            }

            bool findKhongDau = _lColVn.Count > 0 && !findText.CheckVNStringInput();
            ...
            ExpressionEvaluator evaluator = GetExpressionEvaluator(criteria, false, dv);
            bool error = false;
            DataRowView data =  GetDataRowViewSearch(tl, dv, e.Node.Id, out error);
            if (error)
            {
                goto finished;
            }
            ...
            {
                evaluator = GetExpressionEvaluator(criteria, true, dv);
                visible = evaluator.Fit(data);
        ...
        private DataRowView GetDataRowViewSearch(TreeList tl,DataView dv , int id, out bool error)
        {
        ...
        private void OnFixedClick(object sender, EventArgs e)
        {
            ...
            if (!info.Checked)
            {
                for (int i = 0; i < _treeList.VisibleColumns.Count; i++)
                {
                    if (_treeList.VisibleColumns[i].FieldName == fieldNameCheck)
                    {
                        ...
                        if (_treeList.VisibleColumns[i].ParentBand != null)
                        {
                            _treeList.VisibleColumns[i].ParentBand.RootBand.Fixed = FixedStyle.Left;
                        }
                        break;
                    }
                    _treeList.VisibleColumns[i].Fixed = FixedStyle.Left;
            ...
            else if (info.Checked)
            {
                for (int i = _treeList.VisibleColumns.Count - 1; i >= 0; i--)
                {
                    ...
                    if (_treeList.VisibleColumns[i].ParentBand != null)
                    {
                        _treeList.VisibleColumns[i].ParentBand.RootBand.Fixed = FixedStyle.None;
                    }
                }
            }
            //_treeList.VisibleColumns[0].ParentBand.ParentBand.Fixed = FixedStyle.Left;
