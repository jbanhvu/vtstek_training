...
{
    public partial class rpt004PuchaserOrder : DevExpress.XtraReports.UI.XtraReport
    {
        #region "Property"
        ...
        public rpt004PuchaserOrder(Int64 Pk, TreeList tl, DataTable dtHeader, TreeList tlServices, bool isCOdeEn)
        {
            InitializeComponent();
            _tl = tl;
         ...
         DataTable dtLSX=   _inf.LoadLSXNumber(Pk);
            ...
            xrLabelNote.Text = "Unit Price not including VAT";
         ...
         ProcessGeneral.GetSafeString(dtHeader.Rows[0]["InvoiceTaxCode"]));
            ...
            string sPRNO = string.Format("PR No: {0}", ProcessGeneral.GetSafeString(dtHeader.Rows[0]["PRNo"]));
            string sPONo = string.Format("PO No: {0}", ProcessGeneral.GetSafeString(dtHeader.Rows[0]["PONo"]));
                ...
                case "USD":
                    {
                        ...
                        if (isCOdeEn)
                        {
                            xrRichText2.Text = string.Format(@"Say: {0} {1} ", sCurrency, NumberInWord.changeToWords(ProcessGeneral.GetSafeString(TotalUSD)));
                            xrRichText2.Font = new System.Drawing.Font("Tahoma", 8.25F, System.Drawing.FontStyle.Bold);
                            if (totalUnits > 0 && totalUnitsServices <= 0)
                            {
                                xrRichText3.Text = string.Format(@"Total (1): {0:#,#.00} ({1})", ProcessGeneral.GetSafeDouble(TotalUSD), sCurrency);
                            }
                            else if (totalUnits <= 0 && totalUnitsServices > 0)
                            {
                                xrRichText3.Text = string.Format(@"Total (2): {0:#,#.00} ({1})", ProcessGeneral.GetSafeDouble(TotalUSD), sCurrency);
                            }
                            else
                            {
                                xrRichText3.Text = string.Format(@"Total (1 + 2): {0:#,#.00} ({1})", ProcessGeneral.GetSafeDouble(TotalUSD), sCurrency);
                            }
                        ...
                        else
                        {
                            if (totalUnits > 0 && totalUnitsServices <= 0)
                            {
                                xrRichText3.Text = string.Format(@"Thành Tiền (1): {0:#.00} ({1})", TotalUSD, sCurrency);
                            }
                            else if (totalUnits <= 0 && totalUnitsServices > 0)
                            {
                                xrRichText3.Text = string.Format(@"Thành Tiền (2): {0:#.00} ({1})", TotalUSD, sCurrency);
                            }
                            else
                            {
                                xrRichText3.Text = string.Format(@"Thành Tiền (1 + 2): {0:#,#.00} ({1})", TotalUSD, sCurrency);
                            }
                ...
                case "VND":
                    {
                        ...
                        if (isCOdeEn)
                        {
                            ...
                            xrRichText2.Text = string.Format(@"Say : {0} {1}", sCurrency, NumberInWord.changeToWords(ProcessGeneral.GetSafeString(TotalVN)));
                            xrRichText2.Font = new System.Drawing.Font("Tahoma", 8.25F, System.Drawing.FontStyle.Bold);
                            if (totalUnits > 0 && totalUnitsServices <= 0)
                            {
                                xrRichText3.Text = string.Format(@"Total(1): {0:#,##0} ({1})", ProcessGeneral.GetSafeDouble(TotalVN), sCurrency);
                            }
                            else if (totalUnits <= 0 && totalUnitsServices > 0)
                            {
                                xrRichText3.Text = string.Format(@"Total(2): {0:#,##0} ({1})", ProcessGeneral.GetSafeDouble(TotalVN), sCurrency);
                            }
                            else
                            {
                                xrRichText3.Text = string.Format(@"Total(1 + 2): {0:#,##0} ({1})", ProcessGeneral.GetSafeDouble(TotalVN), sCurrency);
                            }
                        ...
                        else
                        {
                            ...
                            //xrRichText3.Text = string.Format(@"Thành Tiền (1 + 2): {0:#,#.000} ({1})", totalUnitsFinal, sCurrency);
                            xrRichText2.Text = string.Format(@"Thành Tiền (Bằng chữ): {0} {1}", sCurrency, NumberInWord.PhanCach(ProcessGeneral.GetSafeString(TotalVN), sCurrency));
                            xrRichText2.Font = new System.Drawing.Font("Tahoma", 8.25F, System.Drawing.FontStyle.Bold);
                            xrRichText3.Font = new System.Drawing.Font("Tahoma", 8.25F, System.Drawing.FontStyle.Bold);
                        }
                    }
